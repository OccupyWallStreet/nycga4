...
class BP_GTM extends BP_Group_Extension {
    ...
    protected function bp_gtm_save_g_discuss($data) {
        ...
        do_action('bp_gtm_save_discussion_post', 'discuss', $_POST['author_id'], $post_id, $_POST['task_id'], $_POST['files_project_id'], $bp->groups->current_group->id);
    ...
    protected function bp_gtm_save_g_new_project($data) {
        ...
        $inserted_project = $wpdb->query($wpdb->prepare("
            ...
            ", $name, $description, $bp->groups->current_group->status, $_POST['project_group'], $_POST['project_creator'], implode(' ', $_POST['user_ids']), $_POST['project_deadline']));
    ...
    protected function bp_gtm_edit_g_project($data) {
        ...
        foreach ((array) $project_resps_old as $username => $misc) { // get the array of all resps
            if ($username != '')
                $resps[] = $username;
        }
        ...
        $updated_project = $wpdb->query($wpdb->prepare("
            ...
            ", $name, $description, $bp->groups->current_group->status, $project_resps, $_POST['project_deadline'], $_POST['project_group'], $project_id));
        ...
        if ($updated_project != null || $updated_cat != null || $updated_tag != null) {
            ...
            bp_core_add_message(__('There was an error updating this project, please try again.', 'bp_gtm'), 'error');
        }
    ...
    protected function bp_gtm_save_g_new_task($data) {
        ...
        $inserted_task = $wpdb->query($wpdb->prepare("
            ...
            ", $name, $description, $status, $_POST['task_parent'], $_POST['task_group'], $_POST['task_creator'], $project_id, implode(' ', $_POST['user_ids']), $_POST['task_deadline']));
    ...
    protected function bp_gtm_edit_g_task($data) {
        ...
        foreach ((array) $task_resps_old as $username => $misc) { // get the array of all resps
            if ($username != '')
                $resps[] = $username;
        }
        $change_roles = array_intersect_key($_POST['user_role'], array_flip($resps));
        ...
        $updated_task = $wpdb->query($wpdb->prepare("
            ...
            WHERE `group_id` = %d AND `id` = %d
            ", $name, $description, $status, $task_resps, $_POST['task_project'], $_POST['task_deadline'], $_POST['task_group'], $task_id));
        ...
        if ($updated_task != null || $updated_cat != null || $updated_tag != null) {
            ...
            bp_core_add_message(__('There was an error updating this task, please try again.', 'bp_gtm'), 'error');
        }
