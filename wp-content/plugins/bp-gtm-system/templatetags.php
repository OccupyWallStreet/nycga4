        ...
        if ($resp->id != 0) {
            ...
            <tr id="<?php echo $resp->id; ?>">
                ...
                <?php if ($bp_gtm['groups'] == 'all' || array_key_exists($bp->groups->current_group->id, $bp_gtm['groups'])) { ?>
                    <td class="td-title">
                        <?php echo!$resp->role_name ? __('Admin', 'bp_gtm') : $resp->role_name; ?>
                    </td>
                <?php } ?>
                <td class="td-poster">
                    <?php $count_tasks = BP_GTM_Resps::get_el_count($resp->id, 'tasks', $bp->groups->current_group->id); ?>
                    <?php echo sprintf(_n('%s task', '%s tasks', $count_tasks, 'bp_gtm'), $count_tasks); ?>
                </td>
                <td class="td-group">
                    <?php
                    $count_projects = BP_GTM_Resps::get_el_count($resp->id, 'projects', $bp->groups->current_group->id);
                    echo sprintf(_n('%s project', '%s projects', $count_projects, 'bp_gtm'), $count_projects);
                    ?>
                </td>
                <td id="users" class="td-freshness">
                    <?php if (bp_gtm_check_access('involved_mention')) { ?>
                        <a href="<?php echo bp_core_get_user_domain($bp->loggedin_user->id, false, false) . BP_ACTIVITY_SLUG . '/?r=' . bp_core_get_username($resp->id, false, false) ?>" title="<?php _e('Mention this user', 'bp_gtm'); ?>">@</a> | 
                    <?php } ?>
                    <?php if (bp_gtm_check_access('involved_pm')) { ?>    
                        <a href="<?php echo bp_core_get_user_domain($bp->loggedin_user->id, false, false) . BP_MESSAGES_SLUG . '/compose/?r=' . bp_core_get_username($resp->id, false, false) ?>" title="<?php _e('Send a private meassage', 'bp_gtm'); ?>"><?php _e('PM', 'bp_gtm'); ?></a>
                    <?php } ?>
                    <?php
                    if (bp_gtm_check_access('involved_email')) {
                        echo ' | ';
                        echo '<a href="#" class="email_notify" id="' . $resp->id . '" title="' . __('Notify user about pending tasks and projects', 'bp_gtm') . '">' . __('N', 'bp_gtm') . '</a>';
                    }
                    ?>
...
function bp_gtm_terms_loop($tags = array(), $tax) {
    global $bp;
    foreach ($tags as $tag) {
        ?>
...
function bp_gtm_term_task_edit_loop($task_id, $tax) {
    ...
    if (count($terms) > 0) {
        ...
        $link = 'terms';
        echo sprintf(__('<p>There are no %ss to display. Create them <a href="%s" target="_blank">here</a></p>', 'bp_gtm'), $tax, $link);
    }
}
            ...
            foreach ($posts as $post) {
                ...
                <li id="post-<?php echo $post->id; ?>" class="alt">
                    <div class="poster-meta">
                        ...
                        <?php echo sprintf(__('%s said %s ago:', 'bp_gtm'), bp_core_get_userlink($post->author_id), bp_core_time_since($post->date_created)) ?>
                    </div>

                    <div class="post-content"><?php echo $post->text; ?></div>

                    <?php do_action('bp_gtm_discuss_after_content', $post); ?>
...
function bp_gtm_project_list_settings() {
    $options = array();
    !empty($_GET['filter']) ? $filter = $_GET['filter'] : $filter = '';
    !empty($_GET['action']) ? $options['action'] = 'project_' . $_GET['action'] : $options['action'] = 'project_' . 'view';
    if ($filter == 'done') {
        $options['page_h4'] = __('Completed Projects', 'bp_gtm');
        $options['projects'] = bp_gtm_get_projects(bp_get_current_group_id(), $filter = 'done');
        $options['done_style'] = 'class="grey"';
    } elseif ($filter == 'alpha') {
        $options['page_h4'] = __('Projects by A/Z', 'bp_gtm');
        $options['projects'] = bp_gtm_get_projects(bp_get_current_group_id(), $filter = 'alpha');
        $options['alpha_style'] = 'class="grey"';
    } elseif ($filter == 'deadline') {
        $options['page_h4'] = __('Projects by Deadline', 'bp_gtm');
        $options['projects'] = bp_gtm_get_projects(bp_get_current_group_id(), $filter = 'deadline');
...
function bp_gtm_task_list_settings($bp_gtm_group_settings) {
    ...
    !empty($_GET['filter']) ? $option['filter'] = $_GET['filter'] : $option['filter'] = '';
    !empty($_GET['action']) ? $option['action'] = $_GET['action'] : $action = 'task_view';
...
add_action('bp_directory_members_actions_loop', 'bp_gtm_display_roles');

function bp_gtm_display_roles($user_role) {
    $roles_list = bp_gtm_roles_list();
    $user_id = bp_core_get_userid($user_role);
    $role_id = bp_gtm_get_role_for_user($user_id);
    echo '<select name="user_role[' . $user_role . ']" class="role-box">';
    foreach ($roles_list as $role) {
        echo '<option value="' . $role->id . '"' . selected($role->id, $role_id->id, true) . '>' . $role->role_name . '</option>';
    };
    echo '</select>';
}
...
function bp_gtm_edit_link($project_id, $gtm_link, $type) {
    ...
    if (bp_gtm_check_access($type_title . '_edit')) { ?>
        <a href="<?php echo $gtm_link . $type . '/edit/' . $project_id ?>" title="<?php _e('Edit this ' . $type_title, 'bp_gtm'); ?>"><img height="16" width="16" src="<?php echo WP_PLUGIN_URL . "/bp-gtm-system/_inc/images/edit.png" ?>" alt="<?php _e('Edit', 'bp_gtm') ?>" /></a>&nbsp;
...
function bp_gtm_view_link($project_id, $project_name, $gtm_link, $type) {
    if (bp_gtm_check_access($type . '_view')) { ?>
        <a class="topic-title" href="<?php echo $gtm_link . $type . 's/view/' . $project_id ?>" title="<?php _e('Permalink: ', 'bp_gtm') . $type; ?>">
...
function bp_gtm_poster_meta($project, $type = 'project') {
    ...
    $action = !empty($_GET['action']) ? $action = $_GET['action'] : $action = $type . '_view';
...
function bp_gtm_view_list_button($gtm_link, $type = 'projects') { ?>
    <a class="button back-task" href="<?php echo $gtm_link . $type; ?>"><?php echo __('&larr; Back: ', 'bp_gtm') . ucfirst($type); ?></a>
    <?php
}

function bp_gtm_edit_button($project_id, $gtm_link, $type) {
    if (bp_gtm_check_access($type . '_edit')) { ?>
        <a class="button" href="<?php echo $gtm_link . $type . 's/edit/' . $project_id; ?>" title="<?php _e('Edit', 'bp_gtm'); ?>"><?php _e('Edit', 'bp_gtm'); ?></a>
        <?php
    }
}
...
function bp_gtm_get_format_date($date) {
    return date_i18n(__('M jS, Y', 'bp_gtm'), strtotime($date));
}
